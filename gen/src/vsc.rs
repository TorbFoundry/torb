...
pub trait GitVersionControl {
    fn create_remote_repo(&self, name: String) -> Result<String, Box<dyn std::error::Error>>;
    ...
    fn create_repo(
        ...
        if local_only {
            ...
            let remote = self.create_remote_repo(name.clone())?;

            Ok((
                self.create_local_repo(&path_buf, Some(name.clone()))?,
                remote,
            ))
        }
    }
...
impl GitVersionControl for GithubVSC {
    fn create_remote_repo(&self, name: String) -> Result<String, Box<dyn std::error::Error>> {
        ...
        */
        ...
        println!("{:?}", req);
        let resp = req.send_json(ureq::json!({
                "name": name,
                "private": true,
                "auto_init": true
            }))?
